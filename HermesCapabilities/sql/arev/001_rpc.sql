-- HermesCapabilities — SQL DÉDIÉ slug arev — 001_rpc.sql
-- Les scripts dédiés au slug (décision utilisateur) : l'agent arev ne peut
-- toucher QUE ses lignes (client_slug='arev' hardcodé — D8-v2).
-- Tables génériques : sql/generic/001_schema.sql (à appliquer AVANT).
...
as $$
    ...
    where d.client_slug = 'arev' and (p_kind is null or d.kind = p_kind)
...
set search_path = public, extensions
as $$
declare v_id uuid; v_action text; v_statut text;
begin
...
-- Grants explicites (EXECUTE par défaut existe ; on le fixe par clarté)
grant execute on function public.rpc_cap_arev_doc_status(text[]) to anon, authenticated;
grant execute on function public.rpc_cap_arev_doc_upsert(text, text, text, extensions.vector, text, text, text, text, jsonb) to anon, authenticated;
